...
@section('content')
    ...
    <section class="main-body-container">
        ...
        <section>
            <form action="{{ route('admin.market.product-categories.update', $productCategory) }}" method="post"
                  id="form">
                @csrf
                @method('PUT')
                <section class="row">
                            ...
                            <select name="parent_id" id="parent_id"
                                ...
                                @forelse($parentCategories as $parentCategory)
                                    <option value="{{ $parentCategory->id }}"
                                            @selected( old('parent_id', $productCategory->parent_id) === $parentCategory->id) class="badge badge-info font-size-12">{{ $parentCategory->name }}</option>
                                    @forelse($parentCategory->children as $subCategory)
                                        <option value=""  disabled readonly @class(['text-secondary' => !$subCategory->parent_id])
                                               class="badge badge-light font-size-12 border-dark">{{ $subCategory->name }}</option>
                                    @empty @endforelse
                                @empty @endforelse
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" id="status"
                                ...
                                @foreach(\App\Enums\CategoryStatus::cases() as $status)
                                    <option @selected(old('status', $productCategory->status->value) === $status->value) value="{{ $status->value }}" >{{ $status->getLabel() }}</option>
                                @endforeach
                            </select>
                        </div>
                    </section>
