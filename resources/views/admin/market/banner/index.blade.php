        ...
        <section class="col-12">
            <section class="main-body-container">
                ...
                <section class="d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom pb-2">
                    <a href="{{ route('admin.market.banners.create') }}"
                       class="btn btn-info  border rounded-pill  btn-hover color-8">ایجاد بنر </a>
                        ...
                        @forelse ($banners as $banner)
                            <tr>
                                ...
                                <td class="shadow-sm text-center">
                                    <img src="{{ asset($banner->image) }}" alt="عکس" width="70" height="50"
                                         class="border">
                                </td>

                                <td>{{ $banner->position->getLabel() }}</td>
                                <td class="width-16-rem text-center">
                                    <a href="{{ route('admin.market.banners.edit', $banner) }}"
                                       class="btn btn-primary btn-sm border rounded-lg btn-hover color-9"><i
                                            class="fa fa-pen font-size-12"></i></a>
                                    <form class="d-inline"
                                          action="{{ route('admin.market.banners.destroy', $banner) }}"
                                          method="post">
                                        @csrf
                                        @method('delete')
                                        <button class="btn btn-danger btn-sm delete border rounded-lg btn-hover color-11"
